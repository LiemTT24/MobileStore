				...
				<div id="myCarousel" class="carousel slide homCar">
					<div class="carousel-inner">
						<c:forEach var="item" items="${ slides }" varStatus="index">
							...
							<img style="width: 60%; margin: 1rem 20% 3rem 20%;"
								src="<c:url value="/assets/user/img/slides/${ item.image }" />"
								alt="bootstrap ecommerce templates">
							<div class="carousel-caption">
								<h4>${item.description}
									<br /> <br /> <span><fmt:formatNumber type="number"
												groupingUsed="true" value="${item.content }" /> VND</span>
								</h4>
							</div>
					</div>
					</c:forEach>
										...
										<li class="span3">
											<div class="thumbnail">
												<a class="zoomTool"
													...
													href="/product-detail/${ item.product_id }"><img
													src="<c:url value="/assets/user/img/slides/${item.image }" />"
				...
				<c:if test="${products.size()>0 }">
					<ul class="thumbnails">
						<c:forEach var="item" items="${products}" varStatus="loop">
							<li class="span4">
								<div class="thumbnail">
									<a class="zoomTool" href="/product-detail/${ item.product_id }"
										title="add to cart"><span class="icon-search"></span>
										QUICK VIEW</a><a href="/product-detail/${ item.product_id }"><img
										src="<c:url value="/assets/user/img/slides/${item.image}" />"
										alt=""></a>
									...
									<h4>
										...
										<a class="shopBtn"
											...
											class="pull-right"> <fmt:formatNumber type="number"
												groupingUsed="true" value="${item.price}" />VND
										</span>
									</h4>
								</div>
							</li>
